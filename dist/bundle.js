...
();function forceLayout() {
    ...
    var simulation = d3.forceSimulation().nodes(nodes).force("link", d3.forceLink(links).distance(10)).force("collide", d3.forceCollide(12)).force('charge', d3.forceManyBody().strength(-50)).force("center", d3.forceCenter(width / 2, height / 2)).force("y", d3.forceY(height / 2)).force("x", d3.forceX(width / 2)

    //add links
    );var link = myChart.selectAll('line').data(links).enter().append('line').attr('stroke', 'white');
    ...
    */
    ...
    }).attr('text-anchor', 'middle').attr('width', '18').attr('height', '12').attr('transform', 'translate(-5,-3)'
    ...
    node.on('mouseover', function (d, i) {
        d3.select(this).select('image').classed('highlight', true);

        tooltip.transition().style('opacity', '1');
        tooltip.html(d['country']).style('left', d3.event.pageX + 8 + 'px').style('top', d3.event.pageY - 30 + 'px');
    ...
    //Whenever time passes, dots must be animated
        ...
        node.attr('transform', function (d, i) {
            return 'translate(' + Math.max(2, Math.min(width - 2, d.x)) + ', ' + Math.max(2, Math.min(height - 2, d.y)) + ')';
        }
